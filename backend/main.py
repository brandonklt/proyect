...
async def get_csv_info(filename: str):
    ...
    try:
        ...
        preview_data_json = df.head(100).to_json(orient='records')
...
async def clean_data_route(request: CleanDataRequest):
    ...
    try:
        ...
        # Registrar archivo limpiado en Supabase
        supabase.table("training_jobs").insert({
...
async def export_to_db(results: ModelResults):
    try:
        ...
        print("Exporting to DB:", db_record)
        ...
        print("Error exporting to DB:", e)
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/reset")
async def reset_session():
    """
    Reinicia la sesión eliminando todos los archivos del directorio de subidas.
    """
    try:
